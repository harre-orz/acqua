...
public:
    ...
    email_address(value_type const & addrspec)
        : namespec(), addrspec(addrspec) {}

    email_address(value_type const & addrspec, value_type const & namespec)
...
inline std::size_t parse_email_address(It beg, It end, Addresses & addrs)
{
...
inline std::size_t parse_email_address(std::string const & text, Addresses & addrs)
{
    return parse_email_address(text.begin(), text.end(), addrs);
}
