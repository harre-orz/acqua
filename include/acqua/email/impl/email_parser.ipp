...
template <typename It, typename CharT, typename Traits = std::char_traits<CharT> >
inline void rfc2047_decode(It beg, It end, std::basic_ostream<CharT, Traits> & os)
{
...
inline void rfc2231_decode(It beg, It end, Params & params)
{
    std::locale const & loc = std::locale::classic();
    std::string oldkey;
    std::string encoded;
    ...
    while(beg != end) {
        ...
        if (qi::parse(beg, end, +(qi::char_ - '*' - '=') >> -('*' >> -(qi::int_ >> '*')) >> '='
            ...
            if (!idx) {
                ...
                rfc2047_decode(val.begin(), val.end(), out);
            } else {
                if (oldkey != newkey) {
                    do {
                        // 古いバッファをカキコ
...
inline void decode_mimeheader(It beg, It end, std::string & str, Params & params)
{
    ...
    rfc2047_decode(beg, it, out);
    if (it != end)
        rfc2231_decode<char>(it, end, params);
}
...
inline void decode_mimeheader(It beg, It end, std::wstring & str, Params & params)
{
    ...
    rfc2047_decode(beg, it, out);
    if (it != end)
        rfc2231_decode<wchar_t>(it, end, params);
}
